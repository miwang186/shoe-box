...
NET_DEVICE_INFO netDeviceInfo = {"", "", 0, 0, 7, 0, 0};
...
_Bool NET_DEVICE_Init(void)
{
	...
	switch(netDeviceInfo.initStep)
	{
		...
		case 12:
			UsartPrintf(USART_DEBUG, "STA Tips:	AT+IPSTART=\"TCP\",\"183.230.40.39\",876\r\n");
			if(!NET_DEVICE_SendCmd("AT+IPSTART=\"TCP\",\"183.230.40.39\",876\r\n","CONNECT")) //连接平台
			{
				Led4_Set(LED_OFF);
				netDeviceInfo.initStep++;
			}
...
_Bool NET_DEVICE_ReLink(void)
{
	...
	UsartPrintf(USART_DEBUG, "ReLink Tips:	AT+IPSTART=\"TCP\",\"183.230.40.39\",876\r\n");

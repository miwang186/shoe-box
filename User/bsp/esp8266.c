...
void SendCmd(char* cmd, char* result, int timeOut)
{
    while(1)
    {
        memset(usart2_rcv_buf,0,sizeof(usart2_rcv_buf));
				usart2_rcv_len=0;
        usart2_write(USART3, (unsigned char *)cmd, strlen((const char *)cmd));
        mDelay(timeOut);
...
void ESP8266_DevLink(const char* devid, const char* auth_key, int timeOut)
{
		...
		usart2_write(USART3,CIPSEND,strlen(CIPSEND));  //向ESP8266发送数据透传指令
		...
		mDelay(200);
		usart2_write(USART3,send_pkg->_data,send_pkg->_write_pos);  //发送设备连接请求数据
		mDelay(500);
		DeleteBuffer(&send_pkg);
		mDelay(200);
		usart2_write(USART3,"+++",3);  //向ESP8266发送+++结束透传，使ESP8266返回指令模式
		mDelay(50);
...
int ESP8266_CheckStatus(int timeOut)
{
		...
		usart2_write(USART3,CIPSTATUS,strlen(CIPSTATUS));
		for(count=0;count<timeOut;count++)
		{
				mDelay(100);
...
{		
		...
		usart2_write(USART3,send_pkg->_data,send_pkg->_write_pos);	//向平台上传数据点
		DeleteBuffer(&send_pkg);
		mDelay(500);

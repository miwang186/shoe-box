 	...
 	if(rval==0)	 
	{
		switch(fx)
		{
			case 0:												//更新UNIGBK.BIN
				...
				ftinfo.ugbksize=174344;					//UNIGBK大小
...
//				break;								//执行错误
			SPI_Flash_Write(tempbuf,offx+flashaddr,4096);		//从0开始写入4096个数据  
	  		offx+=bread;	  
			fupd_prog(x,y,size,write_size,offx);	 			//进度显示
...
//	updata_fontx(x+20*size/2,y,size,unigbk_path,0);			//更新UNIGBK.BIN
//	if(res)return 1;
// 	LCD_ShowString(x,y,240,320,size,"Updating GBK12.BIN  ");
//	updata_fontx(x+20*size/2,y,size,gbk12_path,0);			//更新GBK12.FON
//	if(res)return 2;
	LCD_DispStr(x,y,BLUE,"Updating GBK16.BIN  ");
	updata_fontx(x+20*size/2,y,size,gbk16_path,0);			//更新GBK16.FON
//	if(res)return 3;
